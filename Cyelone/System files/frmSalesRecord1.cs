    ...
    ConnectionString cs = new ConnectionString();
        ...
        private void DataGridView1_RowHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            try
            {
                ...
                frmSales.label6.Text = label10.Text;
        ...
        private void textserchStaues_TextChanged(object sender, EventArgs e)
        {
            try
            {
                con = new SqlConnection(cs.DBConn);
                con.Open();
                cmd = new SqlCommand("SELECT RTRIM(InvoiceNo)as [InvoiceNo],RTRIM(UserName) as [UserName],RTRIM(InvoiceDate) as [InvoiceDate],RTRIM(CustomerID) as [CustomerID],RTRIM(SubTotal) as [SubTotal],RTRIM(VATPer) as [VATPer],RTRIM(VATAmount) as [VATAmount],RTRIM(DiscountPer) as [DiscountPer], RTRIM(DiscountAmount) as [DiscountAmount], RTRIM(GrandTotal) as [GrandTotal], RTRIM(TotalPayment) as [TotalPayment], RTRIM(PaymentDue) as [PaymentDue],RTRIM(PaymentType) as [PaymentType], RTRIM(Status) as [Status], RTRIM(Remarks) as [Remarks]from Invoice_Info where Status like '" + textserchStaues.Text + "%' order by UserName ", con);
